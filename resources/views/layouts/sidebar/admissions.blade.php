...
<p class="font-semibold text-white text-lg">
    {{ auth()->user()->name }}
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="mt-2 w-full px-2 py-2 rounded-lg bg-white text-[#057E2E] font-medium hover:bg-gray-100 transition">
                Logout
            </button>
        </form>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto p-4 text-sm space-y-1">
        ...
        <!-- Admissions -->
        <a href="{{ route('admissions.index') }}" class="flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-white hover:bg-white hover:text-[#057E2E] hover:font-semibold transition">
