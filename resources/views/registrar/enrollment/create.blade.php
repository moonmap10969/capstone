...
<body class="min-h-screen bg-[#F8FAFC] flex" x-data="{ 
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center shrink-0">
            ...
            <button @click="burstMode = !burstMode; selectedStudent = null; selectedIds = []; currentBurstYear = null" 
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <section class="flex-1 overflow-y-auto p-12 bg-[#FBFDFF]">
                <div class="max-w-2xl mx-auto">
                    ...
                    {{-- Single Enrollment --}}
                    <template x-if="!burstMode && selectedStudent">
                        <div class="bg-white p-10 rounded-3xl border border-slate-200 shadow-xl space-y-8 animate-in fade-in zoom-in duration-300">
                            <div>
                                <h2 class="text-3xl font-black text-slate-800" x-text="selectedStudent.studentFirstName + ' ' + selectedStudent.studentLastName"></h2>
                                <p class="text-green-600 font-bold text-sm mt-1 uppercase tracking-widest" x-text="'Processing Grade Level: ' + selectedStudent.year_level"></p>
                            </div>
                            <form action="{{ route('registrar.enrollment.store') }}" method="POST" class="grid grid-cols-2 gap-6">
                                @csrf
                                <input type="hidden" name="admission_ids" :value="JSON.stringify([selectedStudent.id])">
                                
                                <div class="col-span-2 space-y-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Available Sections</label>
                                    <select name="section_id" class="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none focus:border-green-600" required>
                                        <option value="">Select Section...</option>
                                        @foreach($sections as $section) 
                                            <option x-show="selectedStudent && selectedStudent.year_level === '{{ $section->year_level }}'" value="{{ $section->name }}">{{ $section->name }}</option> 
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <button type="submit" class="col-span-2 bg-green-700 hover:bg-green-800 text-white py-5 rounded-2xl font-black transition-all shadow-lg shadow-green-100">Finalize Enrollment</button>
                            </form>
                        </div>
                    </template>

                    {{-- Bulk Enrollment (Burst Mode) --}}
                    <template x-if="burstMode && selectedIds.length > 0">
                        <div class="bg-white rounded-3xl border-2 border-green-500 p-10 space-y-8 animate-in slide-in-from-bottom-4 duration-300">
                            <div class="flex justify-between items-end">
                                <div>
                                    <h3 class="text-2xl font-black text-slate-800">Burst Action</h3>
                                    <p class="text-green-600 font-bold text-sm uppercase tracking-widest">Assigning <span x-text="selectedIds.length"></span> Students to <span x-text="currentBurstYear"></span></p>
                                </div>
                                <span class="bg-green-100 text-green-700 px-4 py-1 rounded-full text-[10px] font-black uppercase">Active</span>
                            </div>
                            <form action="{{ route('registrar.enrollment.store') }}" method="POST" class="grid grid-cols-2 gap-6">
                                @csrf
                                <input type="hidden" name="admission_ids" :value="JSON.stringify(selectedIds)">
                                
<select name="section_id" class="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none focus:border-green-600" required>
        <option value="">Select Section...</option>
        @foreach($sections as $section) 
            <option x-show="selectedStudent && selectedStudent.year_level === '{{ $section->year_level }}'" 
                    value="{{ $section->section_id }}"> {{-- Update this line --}}
                {{ $section->name }}
            </option> 
        @endforeach
    </select>
                                ...
                                <button type="submit" class="col-span-2 bg-green-700 hover:bg-green-800 text-white py-6 rounded-2xl font-black transition-all shadow-xl">Confirm Bulk Enrollment</button>
                            </form>
                        </div>
                    </template>
                </div>
