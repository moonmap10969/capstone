...
class PaymentController extends Controller
{
    ...
    {
        // We eager load 'enrollment.admission' and 'admission' to ensure names show up
        // regardless of whether the payment was linked via Enrollment ID or Student Number.
    ...
    public function store(Request $request)
    {
        ...
        if ($request->payment_method === 'cash') {
            $referenceNumber = 'WALK-IN-' . date('Ymd') . '-' . strtoupper(Str::random(4));
            $receiptPath = 'CASH_PAYMENT';
        } else {
        ...
        if ($request->hasFile('receipt')) {
            ...
            $encryptedContents = AESHelper::encrypt($contents);
        ...
        $payment = clone $tuition->payments()->create([
            'enrollment_id'    => $tuition->enrollment_id,
            'tuition_id'       => $tuition->id,
            'studentNumber'    => $tuition->studentNumber,
            'amount'           => min($request->amount, $tuition->balance), 
    ...
    public function approveOnline($id)
    {
        ...
        $tuition = Tuition::findOrFail($payment->tuition_id);
    ...
    public function show($id)
    {
        ...
        $fullPath = str_contains($dbPath, 'receipts/') ? $dbPath : "receipts/" . $dbPath;
        ...
        return response($decryptedContent, 200)->header('Content-Type', $contentType);
    }
